 ...
 */
public class BenDiView extends FrameLayout {
    ...
    public BenDiView(Context context) {
        this(context, null);
    }

    public BenDiView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BenDiView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        LayoutInflater.from(getContext()).inflate(R.layout.item_bendi_fragment, this, true);
    ...
    public void setPreview(final String filePath,final Bitmap bitmap){
        ...
        post(new Runnable() {
            @Override public void run() {
                // 二次确认
                if(!filePath.equals(BenDiView.this.filePath))return;
                ivPreview.setImageBitmap(bitmap);
            }
        });
    }
